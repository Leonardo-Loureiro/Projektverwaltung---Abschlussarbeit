            ...
            <div id="content">
                ...
                <div class="container">
                    ...
                    if(isset($_SESSION["task_id"]))
                    {
                        ...
                        {
                            require("db_conn.php");
                            $statement_edit = $connection->prepare("SELECT * FROM tasks WHERE task_id = :id");
                            $statement_edit->bindParam(":id", $_SESSION["task_id"]);
                            ...
                            if ($count == 1)
                            {
                                ...
                                echo "<form action = 'edit_task_2.php' method = 'post'>";
                                //Input für Name
                                echo "<p><input type style='width: 25%;'  name='name_taskone' value='" . $row["t_name"] . "'> <strong>Name des Tasks</strong></p>";
                                //Input für Beschreibung
                                echo "<p><strong>Beschreibung</strong><br><textarea style='width: 25%;'  name='description_task'>". $row["t_description"] ."</textarea></p>";
                                //Input für Zieldatum
                                echo "<p><input style='width: 25%;' name='aimdate_task' value='" . $row["t_aimdate"] . "'> <strong>Zieldatum</strong></p>";
                                //Auswahl von verfügbaren Status
                                echo "<p><u><strong>Status</strong></u></p>";
                                $statement2 = $connection->prepare("SELECT * FROM task_status");
                                $statement2->execute();

                                //Alle gefundenen Datensätze auslesen und ausgeben
                                while ($secondrow = $statement2->fetch())
                                {
                                    echo '<p><input id="status_taskone" type="radio" name="status_taskone" value="' . $secondrow["ta_status_id"] . '"> <label for="status_taskone">' .$secondrow["ta_status"]. '</label><br></p>';
                                }                
                                
                                echo "<input type='hidden' name='editing_task' value='" . $_SESSION["task_id"] . "'>";
                        ...
                        //Projektmitarbeiter Bearbeitung
                        if($_SESSION["management_rights"] == 2)
                        {
                            ...
                            if ($count == 1)
                            {
                                ...
                                //Auswahl von verfügbaren Status
                                echo "<p><u><strong>Status</strong></u></p>";
                                //SQL Befehl | Alle Bezeichnung für Task-Status
                                $statement = $connection->prepare("SELECT * FROM task_status");
                                $statement->execute();

                                while ($row = $statement->fetch())
                                {
                                    echo '<p><input id="status_staff_taskone" type="radio" name="status_staff_taskone" value="' . $row["ta_status_id"] . '"> <label for="status_staff_taskone">' .$row["ta_status"]. '</label><br></p>';
                                }
                                
                                echo "<input type='hidden' name='editing_staff_task' value='" . $_SESSION["task_id"] . "'>";
                                echo "<p><input type='submit' value='Speichern'>";
                                echo "</form>";
                            }
                            else
