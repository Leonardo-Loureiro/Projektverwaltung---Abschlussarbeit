                ...
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-md-8" style="height: 67.25px;">
                            <h1 style="margin: 15px 0px;font-size: 31.880000000000006px;color: var(--bs-primary);text-align: left;">Projekte löschen!</h1>
                        </div>
                    </div>
                </div>
                <div class="container">
                    ...
                    {
                        require("db_conn.php");
                        $statement_call = $connection->prepare("SELECT project_id FROM tasks WHERE project_id = :id");
                        $statement_call->bindParam(":id", $_SESSION["project_id"]);
                        $statement_call->execute();
                        $count = $statement_call->rowCount();
                
                        //Keine Tasks gefunden, dann lösche Projekt
                        if ($count == 0)
                        {
                            $statement_delete_proj = $connection->prepare("DELETE FROM projects WHERE proj_id = :projekt");
                            $statement_delete_proj->bindParam(":projekt", $_SESSION["project_id"]);
                            //Das Projekt wird gelöscht execute()
                            $statement_delete_proj->execute();
                            
                        } 
                        //Tasks wurden gefunden
                        if ($count >= 1)
                        {
                            echo "<p>Das Projekt enthält Tasks! 
                                  Mit 'Ja' löschen Sie auch alle dazugehörigen Tasks.
                                  Möchten Sie fortfahren?</p>";
                            echo "<form action= 'delete_project.php' method = 'post'>";
                            echo "<input style= 'color: var(--bs-primary);' type= 'submit' name= 'confirmationone' value= 'Ja'>";
                            echo "</form>";
                
                            //Auf Besttigung warten, wenn ja Projekt mit allen dazu gehörigen Tasks löschen
                            if (isset($_POST["confirmationone"]))
                            {
                                $statement_delete_proj = $connection->prepare("DELETE FROM projects WHERE proj_id = :projekt");
                                $statement_delete_proj->bindParam(":projekt", $_SESSION["project_id"]);
                                //Das Projekt wird gelöscht execute()
                                ...
                                $statement_delete_task = $connection->prepare("DELETE FROM tasks WHERE project_id = :projekt");
                                $statement_delete_task->bindParam(":projekt", $_SESSION["project_id"]);
                                //Das Projekt wird gelöscht execute()
